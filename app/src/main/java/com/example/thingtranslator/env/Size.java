...
package com.example.thingtranslator.env;
...
public class Size implements Comparable<Size>, Serializable {
   ...
   * @return Rotated size.
   */
  public static com.example.thingtranslator.env.Size getRotatedSize(final com.example.thingtranslator.env.Size size, final int rotation) {
    if (rotation % 180 != 0) {
      // The phone is portrait, therefore the camera is sideways and frame should be rotated.
      return new com.example.thingtranslator.env.Size(size.height, size.width);
    }
    return size;
  }

  public static com.example.thingtranslator.env.Size parseFromString(String sizeString) {
    ...
    if (components.length == 2) {
      try {
        ...
        return new com.example.thingtranslator.env.Size(width, height);
  ...
  public static List<Size> sizeStringToList(final String sizes) {
    ...
    if (sizes != null) {
      ...
      for (final String pair : pairs) {
        final com.example.thingtranslator.env.Size size = com.example.thingtranslator.env.Size.parseFromString(pair);
        if (size != null) {
          sizeList.add(size);
        }
      }
  ...
  @Override
  public int compareTo(final com.example.thingtranslator.env.Size other) {
  ...
  public boolean equals(final Object other) {
    ...
    if (!(other instanceof com.example.thingtranslator.env.Size)) {
      return false;
    }

    final com.example.thingtranslator.env.Size otherSize = (com.example.thingtranslator.env.Size) other;
